...
export const filtersFromQuery = (query) => ({
  ...
  remote: query.remote === 'true' ? true : undefined,
...
export const queryFromFilters = (filters) => {
  ...
  if (filters.remote === true) query.remote = 'true'
  if (filters.salaryMin) query.min_salary = String(filters.salaryMin)

  if (filters.page > 1) query.page = String(filters.page)
  if (filters.sortBy && filters.sortBy !== DEFAULT_SORT_BY) query.sort_by = filters.sortBy
  if (filters.sortOrder && filters.sortOrder !== DEFAULT_SORT_ORDER) query.sort_order = filters.sortOrder
