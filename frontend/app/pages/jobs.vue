      ...
      <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <div class="flex items-center gap-8">
          ...
          <nav class="hidden md:flex items-center gap-6">
            <NuxtLink to="/jobs" class="text-gray-900 font-semibold border-b-2 border-blue-600 pb-1">Browse Jobs</NuxtLink>
            <NuxtLink to="#" class="text-gray-500 hover:text-gray-900 transition-colors">Companies</NuxtLink>
            <NuxtLink to="#" class="text-gray-500 hover:text-gray-900 transition-colors">Salaries</NuxtLink>
          </nav>
        </div>
        <div class="flex items-center gap-4">
          <button 
            @click="handleSync" 
            :disabled="isSyncing"
            class="hidden sm:flex items-center gap-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 px-4 py-2 rounded-lg transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" :class="{ 'animate-spin': isSyncing }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
            {{ isSyncing ? 'Syncing...' : 'Sync Jobs' }}
          </button>
          <button class="bg-gray-900 text-white px-5 py-2 rounded-lg font-semibold hover:bg-gray-800 transition-colors">
            Sign In
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3">
          ...
          <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-8 flex flex-col sm:flex-row gap-4">
            <div class="flex-grow relative">
              ...
              <input 
                ...
                class="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
              />
            </div>
            <div class="flex-grow relative">
              <span class="absolute inset-y-0 left-3 flex items-center text-gray-400">
            ...
            <button 
              @click="handleSearch"
              class="bg-blue-600 text-white px-8 py-2.5 rounded-lg font-bold hover:bg-blue-700 transition-all active:scale-[0.98]"
            >
              Search
            </button>
          </div>
          ...
          <div v-if="pending" class="flex flex-col gap-4">
            <div v-for="i in 5" :key="i" class="bg-white border border-gray-200 rounded-xl p-6 h-48 animate-pulse shadow-sm">
          ...
          <div v-else-if="error" class="bg-red-50 border border-red-200 text-red-700 p-8 rounded-xl text-center shadow-sm">
          ...
          <div v-else class="space-y-6">
            <div v-if="jobs?.length > 0" class="flex flex-col gap-6">
              ...
              <div v-if="metadata && metadata.total_pages > 1" class="flex justify-center items-center gap-4 mt-8 py-4">
                <button 
                  ...
                  class="p-2 rounded-lg border border-gray-300 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-white hover:shadow-sm transition-all"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div v-else class="bg-white border border-gray-200 rounded-xl p-12 text-center shadow-sm">
              ...
              <p class="text-gray-600">Try adjusting your filters or search terms to find what you're looking for.</p>
            </div>
          </div>
        </div>
      </div>
